...
void print_queue(struct Queue *queue) {
    ...
    while (element) {
        printf("%d id:%d | timestamp:%-5d | c:%d  \n",rank_comm, element->src_id, element->timestamp, element->cuchy);
...
struct part* create_part(int timestamp, int src_id, int cuchy) {
    ...
    new_elemet->timestamp = timestamp;
    return new_elemet;
}

void add_by_time(struct Queue *queue, int timestamp, int src_id, int cuchy) {
    struct part *new_element = create_part(timestamp, src_id, cuchy);  //dane o oTAku od którego otrzymaliśmy
    ...
    while ((head->next) && (head->timestamp < new_element->timestamp || ((head->timestamp == new_element->timestamp) && (head->src_id < new_element->src_id)))) { //przesuwamy nowy element wkolejce do przodu dopóki on ma wjększą wartość zegara od pozostałch
        head = head->next;
    }

    // nie znaloziono z większym znacznikiem - wstaw na koniec
    if ((head->timestamp < new_element->timestamp) || (head->timestamp == new_element->timestamp && head->src_id < new_element->src_id)) {
...
struct part* get_info_by_pos(struct Queue *queue, int id) {
    struct part *head = queue->head;
    if(is_empty(queue)) {
